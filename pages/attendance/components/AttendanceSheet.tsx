...
export default function AttendanceSheet() {
  ...
  const fetchData = async (type: EventType) => {
    ...
    try {
      ...
      setUsers(
        cachedUsers.map((u) => ({ uid: u.uid, displayName: u.displayName, classYear: u.classYear }))
      );
    ...
    return (
      <Box sx={{ p: 3, textAlign: 'center' }}>
        ...
        <Typography variant="body2" color="textSecondary">
          The attendance sheet is best viewed on a tablet or larger device. Please switch to a
          larger screen.
        </Typography>
      </Box>
    );
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <TextField
          ...
          sx={{ width: 160 }}
        />
        <Button variant="outlined" size="small" onClick={() => fetchData(activeType)}>
          Apply
        </Button>
        <Button
          size="small"
          ...
          <Table size="small" stickyHeader>
            <TableHead>
              <TableRow>
                ...
                {events.map((ev) => (
                  <TableCell key={ev.id} align="center" sx={{ minWidth: 56, whiteSpace: 'nowrap' }}>
                    {formatColDate(ev.startDate)}
                  </TableCell>
                ))}
              </TableRow>
            </TableHead>
                      ...
                      return (
                        <TableCell key={ev.id} align="center" padding="none">
                          <Tooltip title={status ?? 'Not Logged'} placement="top">
                            <Button
                              onClick={() => handleCellClick(user.uid, ev.id)}
                              sx={{
                                minWidth: 48,
                                height: 32,
                                borderRadius: 0,
                                ...
                                fontSize: '0.65rem',
                                '&:hover': {
                                  bgcolor: status ? STATUS_COLORS[status] : 'action.hover',
                                  opacity: 0.85,
                                },
                              }}
                            >
