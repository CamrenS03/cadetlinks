        ...
        <AppTheme {...props}>
            ...
            <AppContainer direction="column" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                { selected ? (
                    ...
                    <Box sx={{ flex: 1, width: '100%', maxWidth: 680, mx: 'auto', p: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
                        <Typography variant='h5' sx={{ fontWeight: 600 }}>Profile Lookup</Typography>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                            <Typography variant='body2' color='textSecondary'>Flight:</Typography>
                        ...
                        {filtered.length === 0 ? (
                            <Typography variant='body2' color='textSecondary'>No cadets match your search.</Typography>
                        ) : (
                                    ...
                                    return (
                                        <ListItem key={user.uid} disablePadding>
                                            <Card variant='outlined' sx={{ width: '100%' }}>
                                                <CardActionArea onClick={() => setSelected(user)} sx={{ justifyContent: 'left' }}>
                                                    <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2, py: 1.5 }}>
                                                        <Avatar src={user.photoURL} sx={{ width: 44, height: 44, bgcolor: 'primary.main', fontSize: 16 }}>
                                                            {!user.photoURL && <Person2Outlined />}
                                                        </Avatar>
                                                        <Box sx={{ flex: 1, minWidth: 0 }}>
                                                            <Typography variant='body1' sx={{ fontWeight: 600 }} noWrap>{user.displayName}</Typography>
