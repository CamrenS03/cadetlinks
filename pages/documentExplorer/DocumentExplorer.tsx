...
export default function DocumentExplorer(props: { disableCustomTheme?: boolean }) {
    ...
    const [breadcrumbs, setBreadcrumbs] = useState<BreadCrumbEntry[]>([{ id: null, name: 'Documents' }]);
    ...
    const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        task.on(
            ...
            (err) => { console.error(err); setError('Upload failed'); setUploadProgress(null); },
            async () => {
        ...
        <AppTheme {...props}>
            ...
            <AppContainer direction="column" sx={{ justifyContent: 'flex-start', alignItems: 'center' }}>
                ...
                <Box sx={{ width: '100%', maxWidth: 800, mt: 2, px: 2 }}>
                    ...
                    <Breadcrumbs sx={{ mb: 2 }}>
                        {breadcrumbs.map((crumb, i) =>
                            i < breadcrumbs.length - 1 ? (
                                ...
                                <Typography key={crumb.id ?? 'root'} color='text.primary'>{crumb.name}</Typography>
                            )
                        )}
                    </Breadcrumbs>
                    ...
                    {uploadProgress !== null && (<Alert severity='info' sx={{ mb: 1}}>Uploading... {uploadProgress}%</Alert>)}
                ...
                <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={handleMenuClose}>
                    <MenuItem onClick={() => { setRenameName(menuItem?.name ?? ''); setOpenRename(true); handleMenuClose(); }}>Rename</MenuItem>
                    <MenuItem onClick={() => { setOpenDeleteConfirm(true); handleMenuClose(); }} sx={{ color: 'error.main' }}>Delete</MenuItem>
                </Menu>

                {/* New Folder Dialog */}
                <Dialog open={openNewFolder} onClose={() => setOpenNewFolder(false)} maxWidth='xs' fullWidth>
                ...
                <Dialog open={openDeleteConfirm} onClose={() => setOpenDeleteConfirm(false)}>
                    ...
                    <DialogActions>
                        ...
                        <Button onClick={handleDelete} variant="contained">Rename</Button>
                    </DialogActions>
                </Dialog>
                ...
                <Dialog open={openPDF} onClose={() => setOpenPDF(false)} maxWidth='lg' fullWidth>
                    <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography noWrap sx={{ flex: 1, mr: 1 }}>{pdfItem?.name}</Typography>
                        <Box sx={{ display: 'flex', gap: 1, flexShrink: 0 }}>
                            {pdfItem?.downloadURL && (
                                <Tooltip title="Download">
                                    <IconButton component='a' href={pdfItem.downloadURL} download={pdfItem.name} target='_blank'><DownloadIcon /></IconButton>
                                </Tooltip>
                            )}
